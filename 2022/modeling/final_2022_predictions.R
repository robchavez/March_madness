...
Dataset_test <- both_data %>% 
  filter(season != 2022, is.na(seed)==FALSE, is.na(seed.1)==FALSE) %>% 
  select(-season, 
         -School,  
         -wteam, 
         -season.1, 
...
test_data <- read_csv("~/Google Drive/March_madness/March_madness-master/2022/seeded_all_regseason_cleaned_1996-2022.csv")
test_data <- test_data %>% filter(season == "2022") %>% filter(is.na(seed)==F)
...
write.csv(prediction_data,"~/Google Drive/March_madness/March_madness-master/2022/Processed_data/prediction_data_2022.csv")

#Predict class probabilities (i.e. 'certainty' scores)

prediction_data_sub <- prediction_data %>% 
  filter(season ==2022) %>% select(-season, -School, -season.1,   -School.1)
...
write.csv(output_predictions, "~/Google Drive/March_madness/March_madness-master/2022/model_predictions_2022.csv")
...
write_csv(clean_pred_sub, "~/Google Drive/March_madness/March_madness-master/2022/Processed_data/final_model_predictions_2022.csv")
